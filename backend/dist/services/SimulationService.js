...
class SimulationService extends events_1.EventEmitter {
    ...
    async simulateSSHBruteForce(params) {
        ...
        logger_1.logger.info(`Starting SSH brute force simulation: ${attackerIp} → ${target}`);
    ...
    async simulateLateralMovement(params) {
        ...
        const attackerIp = `10.20.20.${Math.floor(Math.random() * 100) + 100}`;
        const attackerId = `APT-${attackerIp.replace(/\./g, '-')}`;
        logger_1.logger.info(`Starting lateral movement simulation: ${source} → [${targets.join(', ')}]`);
    ...
    async simulateCredentialTheft(params) {
        ...
        const attackerIp = `10.20.20.${Math.floor(Math.random() * 100) + 100}`;
        const attackerId = `APT-${attackerIp.replace(/\./g, '-')}`;
        logger_1.logger.info(`Starting credential theft simulation: ${attackerIp} → ${target} (${tool})`);
        // Create attacker
        const attacker = new models_1.Attacker({
            attackerId,
            ipAddress: attackerIp,
    ...
    async simulateDiscovery(params) {
        ...
        const attackerIp = `10.20.20.${Math.floor(Math.random() * 100) + 100}`;
        const attackerId = `APT-${attackerIp.replace(/\./g, '-')}`;
        logger_1.logger.info(`Starting discovery simulation: ${attackerIp} from ${source}`);
        const attacker = new models_1.Attacker({
            attackerId,
            ipAddress: attackerIp,
            entryPoint: source,
    ...
    async simulatePrivilegeEscalation(params) {
        ...
        const attackerIp = `10.20.20.${Math.floor(Math.random() * 100) + 100}`;
        const attackerId = `APT-${attackerIp.replace(/\./g, '-')}`;
        logger_1.logger.info(`Starting privilege escalation simulation: ${attackerIp} → ${target}`);
        const attacker = new models_1.Attacker({
            attackerId,
            ipAddress: attackerIp,
            entryPoint: target,
    ...
    async simulateFullCampaign(params) {
        ...
        const attackerIp = `10.20.20.${Math.floor(Math.random() * 100) + 100}`;
        const attackerId = `APT-${attackerIp.replace(/\./g, '-')}`;
        logger_1.logger.info(`Starting full campaign simulation: ${attackerIp} (complexity: ${complexity})`);
        const attacker = new models_1.Attacker({
            attackerId,
            ipAddress: attackerIp,
     ...
     */
    async triggerSync() {
        logger_1.logger.info('Manual CRDT sync triggered via API');
